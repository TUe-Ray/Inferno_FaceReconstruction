...
def main():
    ...
    parser.add_argument('--input_folder', type=str, default= str(Path(get_path_to_assets())/ "/home/inferno/src/inferno/inferno_apps/FaceReconstruction/demo/TestSamples/clear_bg"))
    parser.add_argument('--output_folder', type=str, default="image_output/clear_bg", help="Output folder to save the results to.")
    ...
    for i in auto.tqdm( range(len(dataset))):
        ...
        for j in range(current_bs):
            ...
            # if args.save_depth:
            #     try:
            #         depth_image = Renderer.render_depth(opdict['trans_verts']).repeat(1,3,1,1)
